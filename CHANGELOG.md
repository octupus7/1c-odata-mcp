...
- **Подбор банковского счёта организации был сломан молча.** Справочник
  ...
  в `create_payment` и `create_bank_writeoff` не работал никогда. Теперь выборка
...
- **Счёт-фактура на акт услуг получал нулевой НДС.** `invoiceBasis` — чтение
  документа-основания — брало строки только из ТЧ «Товары». Акт, созданный через
  `create_act`, живёт в «Услугах», поэтому `create_issued_invoice` и
  `create_received_invoice` на таком основании выставляли `СуммаНДСДокумента: 0`
  вместо реального налога. Теперь НДС собирается по обеим табличным частям.
  (Тот же класс, что и правка строк выше, но в другом месте.)
- **Сумма документа со смешанными табличными частями.** У реализации бывают
  заполнены и «Товары», и «Услуги» одновременно. Правка строк и `copy_document`
  считали `СуммаДокумента` только по той ТЧ, которую трогают, — итог занижался на
  сумму второй. Теперь вторая ТЧ остаётся как есть, но входит в сумму документа.
- **Сумма НДС строки не теряется при правке.** При пересборке табличной части
  `СуммаНДС` отбрасывалась всегда, и у документа с НДС строки уезжали в 1С с
  нулевым налогом. Теперь она переносится, если количество, цена и ставка не
  менялись (тогда прежний налог верен). Пересчитывать её мы намеренно не беремся:
  формула зависит от `СуммаВключаетНДС` документа и процента ставки — угадывать тут
  нельзя, а при изменившихся слагаемых значение задаётся явно.
